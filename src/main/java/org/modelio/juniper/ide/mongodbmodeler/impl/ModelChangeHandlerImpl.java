...
public class ModelChangeHandlerImpl implements IModelChangeHandler {
	...
	private void visitObject(IModelingSession session, MObject object)
		...
		if (object instanceof Package
			...
			if(((Dependency) object).getImpacted().isStereotyped("MongoDBModeler","MongoDBServer")){
				ModelElement datamodel = ((Dependency) object).getDependsOn();
				session.getModel().createStaticDiagram("Diagram",datamodel,"MongoDBModeler", "MongoDBDataModelDiagram");
			}
		}else if (object instanceof StaticDiagram
				&& (((ModelElement) object).isStereotyped("MongoDBModeler",
						"MongoDBDataModelDiagram"))) {
