...
public abstract class RunScriptCommand extends DefaultModuleCommandHandler {
	...
	public void actionPerformed(List<MObject> selectedElements, IModule module) {
		...
		try (ITransaction transaction = session.createTransaction("RunScript-"
				+ scriptPath)) {
			runScript(module, scriptPath, selectedElements);
			transaction.commit();
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	private static String runScript(IModule module, String scriptName,
			List<MObject> selectedElements) throws FileNotFoundException,
			ScriptException {
